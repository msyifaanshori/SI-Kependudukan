...
@section('title', 'Edit Penduduk - SI Kependudukan')

@section('content')
<div class="bg-white p-6 md:p-8 rounded-lg shadow-md max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Data Penduduk</h1>
    ...
    <form action="{{ route('penduduk.update', $penduduk->nik) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                ...
                <input type="text" value="{{ $penduduk->nik }}" readonly class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100">
            </div>

            <div>
                <label class="block text-gray-700 font-medium mb-2">Nama Lengkap</label>
                <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $penduduk->nama_lengkap) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nama_lengkap') border-red-500 @enderror">
                @error('nama_lengkap')
            ...
            <div>
                ...
                <select name="id_pendidikan" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('id_pendidikan') border-red-500 @enderror">
                    ...
                    @foreach($pendidikan as $p)
                        <option value="{{ $p->id_pendidikan }}" {{ old('id_pendidikan', $penduduk->id_pendidikan) == $p->id_pendidikan ? 'selected' : '' }}>{{ $p->jenjang }}</option>
                    @endforeach
                </select>
                @error('id_pendidikan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <select name="id_pekerjaan" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('id_pekerjaan') border-red-500 @enderror">
                    ...
                    @foreach($pekerjaan as $p)
                        <option value="{{ $p->id_pekerjaan }}" {{ old('id_pekerjaan', $penduduk->id_pekerjaan) == $p->id_pekerjaan ? 'selected' : '' }}>{{ $p->nama_pekerjaan }}</option>
                    @endforeach
                </select>
                @error('id_pekerjaan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
